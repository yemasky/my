...
class Encrypt {
	...
	function encode($string, $key = '')
	{
		...
		$enc = str_replace('+', '-', base64_encode($enc));
		$enc = str_replace('=', '', $enc);
		//$enc = str_replace("/", '?', $enc);
		//$enc = str_replace("\013", '#', $enc);
		//$enc = str_replace("\014", '$', $enc);
		return $enc;
	}
	...
	function decode($string, $key = '')
	{
		...
		//$string = str_replace("?", "/", $string);
		//$string = str_replace('#', "\013", $string);
		//$string = str_replace('$', "\014", $string);
		$key = $this->get_key($key);
	...
	function encode_from_legacy($string, $legacy_mode = MCRYPT_MODE_ECB, $key = '')
	{
		...
		{
			throw new Exception('error Encoding from legacy is available only when Mcrypt is in use.');
			return FALSE;
		}
